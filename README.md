...
| [kennel-wars/](kennel-wars/) | *Kennel Wars* — breed a war-dog army, fortify your kennel-castle, and raid rival lords. Single-player, but built on a **pure, deterministic battle simulation** (no DOM) so async PvP is an extension rather than a rewrite; `node tools/verify.js` re-runs raids server-side to prove it. | `open kennel-wars/index.html` |
