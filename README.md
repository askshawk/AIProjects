...
| [broken-collars/](broken-collars/) | *Broken Collars* — an **isometric** war-dog strategy game: breed a pack, fortify your kennel-castle, and raid rival lords who cage stolen hounds. Break their cages and those dogs join you. All art drawn procedurally in canvas; built on a **pure, deterministic battle simulation** (no DOM) so async PvP is an extension rather than a rewrite, with `node tools/verify.js` re-running raids server-side to prove it. | `open broken-collars/index.html` |
| [imperium-online/](imperium-online/) | *Imperium Online* — a Roman-era **async multiplayer** city builder (Grepolis-style): server-authoritative state, on-demand "catch-up" simulation (timers keep ticking while you're offline), and a shared world map. **FastAPI + SQLModel** backend, **Next.js + Phaser** client. | See [imperium-online/README.md](imperium-online/README.md) (2 terminals) |
